package edu.informatica3.lucas_antun.practico_integrador.main;

import edu.informatica3.lucas_antun.practico_integrador.enums.*;
import edu.informatica3.lucas_antun.practico_integrador.estructuras.*;
...
import edu.informatica3.lucas_antun.practico_integrador.utils.Pila;
...
public class SistemaGestionTurnosMedicos {
    ...
    private void ejercicio2_VerAgendaMedico() {
        ...
        Lista<Turno> turnos = agenda.obtenerTurnosOrdenados();
        if (turnos.isEmpty()) {
    ...
    private void ejercicio9_AuditoriaUndoRedo() {
        ...
        // Crear pilas para Undo/Redo (implementación propia)
        Pila<OperacionHistorial> pilaUndo = new Pila<>();
        Pila<OperacionHistorial> pilaRedo = new Pila<>();
            ...
            try {
                ...
                switch (opcion) {
                    case 1: // Agregar turno
                        ...
                        if (agenda.agendar(nuevoTurno)) {
                            ...
                            pilaUndo.push(new OperacionHistorial(
                                TipoOperacion.AGREGAR, nuevoTurno, desc));
                    ...
                    case 2: // Cancelar turno
                        ...
                        Lista<Turno> turnosListaCancelar = agenda.obtenerTurnosOrdenados();
                        for (int i = 0; i < turnosListaCancelar.size(); i++) {
                            Turno t = turnosListaCancelar.get(i);
                        ...
                        if (turnoCancelar != null) {
                            if (agenda.cancelar(idCancelar)) {
                                ...
                                pilaUndo.push(new OperacionHistorial(
                                    TipoOperacion.CANCELAR, turnoCancelar, desc));
                                pilaRedo.clear();
                    ...
                    case 3: // Reprogramar turno
                        ...
                        Lista<Turno> turnosListaReprog = agenda.obtenerTurnosOrdenados();
                        for (int i = 0; i < turnosListaReprog.size(); i++) {
                            Turno t = turnosListaReprog.get(i);
                    ...
                    case 4: // UNDO
                        if (pilaUndo.isEmpty()) {
                            ...
                            // Usar if-else en lugar de switch para evitar clase anónima
                            if (op.tipo == TipoOperacion.AGREGAR) {
                                // Deshacer AGREGAR = cancelar turno
                                agenda.cancelar(op.turno.getId());
                            } else if (op.tipo == TipoOperacion.CANCELAR) {
                                // Deshacer CANCELAR = reagregar turno
                                agenda.agendar(op.turno);
                            } else if (op.tipo == TipoOperacion.REPROGRAMAR) {
